<?php
  ...
  $invoice = new phpinvoice('corporate','A4', '$');

  // Header Settings 
  $invoice->setLogo("../templates/corporate/logo.jpg");
  $invoice->setType("Purchase Invoice");
  $invoice->setReference("INV-55033645");
  $invoice->setDate(date('M dS ,Y',time()));
  $invoice->setDue(date('M dS ,Y',strtotime('+3 months')));
  $invoice->setFrom(array("100 Cool Heights, XY 80000, US","(602) 519-0450","company@example.com"));
  $invoice->setTo(array("796 Silver Harbour,","TX 79273, US","john@example.com"));
  ...
  // Adding additional parameters to html invoice.
  $invoice->company_name = "Company XYZ";
  ...
  echo $invoice->render('example3.pdf','I'); /* I => Display on browser, D => Force Download, F => local path save, S => return document path */
?>