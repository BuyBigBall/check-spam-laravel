...
<section class="home d-flex align-items-center">
  ...
  <div class="container">
    <div class="row justify-content-center">
      <div class="col-md-7">
        <h1 class='main-title'>{{__('Mailbox Small Title')}}</h1>
        <div class="home-text">
          <form method="POST" action="{{ route('spamtest') }}">
          @csrf
          <h1>{{__('First, send your email to:')}}</h1>
          <div class="custom-email">
            ...
            <button type="button" data-toggle="tooltip" data-placement="bottom" title="{{__('Click To Copy!')}}"
          ...
          <div class="home-btn" style='display:none;'>
            <div class="row align-items-center">
              <div class="col text-center"><a href="{{route('home')}}" class="btn btn-1"><i class="fas fa-redo-alt"></i> {{__('Refresh')}}</a></div>
              <div class="col text-center"><a href="{{route('change','#changeaccount')}}" class="btn btn-1"><i class="fas fa-pencil-alt"></i> {{__('Change')}}</a></div>
              <div class="col text-center"><a 
                ...
                @else
                  href="{{route('delete')}}" 
                @endif
                class="btn btn-1" ><i class="far fa-trash-alt"></i> {{__('Delete')}}</a></div>
            </div>
          </div>
          
          <div class="counter">
            <button type="submit" class="btn btn-2"><b>{{__('Then check your score')}}</b></button>
            <!-- 
            <span class=" count_ mail_count">
              <b>{{__('Emails Created')}}</b>
              <em class="css_spirite">{{$setdata['emails_created'] + $setdata['total_emails_created']}}</em>
            </span>
            <span class=" count_ mail_count">
              <b>{{__('Messages Received')}}</b>
